...
function setVerifyVisible(show) { document.getElementById("verify-email-form").classList.toggle("hidden", !show); }
...
document.getElementById("signup-form").addEventListener("submit", async (e) => {
  ...
  try {
    const { error } = await supabaseClient.auth.signUp({
      email, password,
      options: { data: { full_name: name }, emailRedirectTo: PUBLIC_URL + "auth.html" }
    });
    if (error) {
    ...
    setVerifyVisible(true);
    showAuth(`Your verification code was sent to <b>${safe(email)}</b>. Use the newest code from the latest email.`, true);
  } catch (err) {
...
document.getElementById("verify-email-form").addEventListener("submit", async (e) => {
  ...
  try {
    ...
    if (error) {
      if (/expired|invalid|incorrect/i.test(error.message || "")) return showAuth("That code is expired or invalid. Tap ‘Send a new code’ and use the newest email.");
  ...
  } catch (err) { showAuth("We could not verify the code. Please try again."); }
  finally { button.disabled = false; }
});
...
document.getElementById("resend-code-button").addEventListener("click", async () => {
  ...
  const button = document.getElementById("resend-code-button"); button.disabled = true;
  try {
    const { error } = await supabaseClient.auth.resend({
      type: "signup",
      email,
      options: { emailRedirectTo: PUBLIC_URL + "auth.html" }
    });
    if (error) {
      if (/rate limit|too many|security/i.test(error.message || "")) return showAuth("Please wait a little before requesting another code. Check your inbox and spam/junk first.");
      return showAuth(error.message || "We could not resend the code. Please try again.");
    }
    document.getElementById("verify-code").value = "";
    showAuth(`A new verification code was sent to <b>${safe(email)}</b>. Use the newest email only.`, true);
  } catch (err) { showAuth("Resend could not connect to the account service. Check your internet connection and try again."); }
  finally { button.disabled = false; }
});

document.getElementById("back-to-auth").addEventListener("click", () => { setVerifyVisible(false); showAuth("Create an account or sign in to continue."); });
...
document.getElementById("login-form").addEventListener("submit", async (e) => {
  ...
  } catch (err) { showAuth("We could not sign you in. Check your connection and try again."); }
  finally { button.disabled = false; }
});
...
document.getElementById("reset-button").addEventListener("click", async () => {
  ...
  } catch (err) { showAuth("We could not send the reset email. Please try again."); }
});
...
async function signInWithProvider(provider) {
  ...
  } catch (err) { showAuth(`Could not start ${provider} sign-in. Check your connection and provider setup.`); }
}
